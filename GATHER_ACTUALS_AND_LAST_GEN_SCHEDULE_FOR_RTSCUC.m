...
LAST_PUMPSTATUS_ACTUAL_VAL(ACTUAL_PUMP_OUTPUT_VAL>0)=1;
    ...
    if time - PRTC/60 < ACTUAL_PUMP(1,1)
        ...
        STORAGEVALUE_VAL(:,initial_storage) = ACTUAL_STORAGE_LEVEL(AGC_interval_index-round(PRTC*60/t_AGC),2:1+nESR)' + LAST_PUMP_SCHEDULE_VAL.*STORAGEVALUE_VAL(:,efficiency).*tRTC./60 - LAST_GEN_SCHEDULE_VAL(storage_to_gen_index).*tRTC./60;
    end; 

...
minutes_where_shutdown_needs_considering = (GENVALUE_VAL(:,capacity) - GENVALUE_VAL(:,min_gen))./GENVALUE_VAL(:,ramp_rate)+60.*GENVALUE_VAL(:,sd_time);%in minutes
REDUCE_OUTPUT_PREPARE_SHUTDOWN_VAL=GENVALUE_VAL(:,capacity)*ones(1,HRTD);
REDUCE_OUTPUT_PREPARE_SHUTDOWN_indices = [];
for i=1:ngen
    if 60*(RTD_LOOKAHEAD_INTERVAL_VAL(HRTD,1)-RTD_LOOKAHEAD_INTERVAL_VAL(1,1))< minutes_where_shutdown_needs_considering(i,1) && LAST_STATUS_VAL(i,1) == 1 ...
        && (RTSCUCSTART_YES(i,1)==0 ||  minutes_where_shutdown_needs_considering(i,1) < HRTC*IRTC)
    look_for_time_end = min(size(STATUS,1),ceil((time + minutes_where_shutdown_needs_considering(i,1)/60)*rtscuc_I_perhour-eps)+1); 
    if any(STATUS(RTSCUC_binding_interval_index:look_for_time_end,1+i) == 0)
        ...
        REDUCE_OUTPUT_PREPARE_SHUTDOWN_indices =[REDUCE_OUTPUT_PREPARE_SHUTDOWN_indices;i];
        for t=1:HRTD
            ...
            elseif SDForce_time_shutdown >= RTD_LOOKAHEAD_INTERVAL_VAL(t,1)
...
REDUCE_OUTPUT_PREPARE_SHUTDOWN.val = REDUCE_OUTPUT_PREPARE_SHUTDOWN_VAL(REDUCE_OUTPUT_PREPARE_SHUTDOWN_indices,:)./SYSTEMVALUE_VAL(mva_pu);
...
REDUCE_OUTPUT_PREPARE_SHUTDOWN.uels = {FORCE_SHUTDOWN_GEN_VAL' INTERVAL_VAL'};

...
for i=1:ngen
    if GENVALUE_VAL(i,initial_status) == 0
        for t=1:HDAC
            if t*IDAC <= GENVALUE_VAL(i,md_time) - GENVALUE_VAL(i,initial_hour)
        ...
        for t=1:HDAC
            if t*IDAC <= GENVALUE_VAL(i,mr_time) - GENVALUE_VAL(i,initial_hour)
...
if size(STORAGEVALUE.uels{1, 1},1) > 0 && ~isempty(storage_to_gen_index)
    for i=1:size(storage_to_gen_index,1)
        if STORAGEVALUE_VAL(i,initial_pump_status) == 0
            for t=1:HDAC
                if t*IDAC <= GENVALUE_VAL(storage_to_gen_index(i),md_time) - STORAGEVALUE_VAL(i,initial_pump_hour)
            ...
            for t=1:HDAC
                if t*IDAC <= STORAGEVALUE_VAL(i,min_pump_time) - STORAGEVALUE_VAL(i,initial_pump_hour)

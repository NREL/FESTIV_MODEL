...
if strcmp(SIMULATE_CONTINGENCIES,'YES') == 1
    if Contingency_input_check == 1
        ...
        gen_repair_time=gen_outage_time+GENVALUE.val(:,mttr);
        gen_repair_time(gen_repair_time==inf)=-inf;
    else
        for i=1:ngen
            full_outage = GENVALUE.val(i,forced_outage_rate)/GENVALUE.val(i,mttr);%full outage probability.
            h=1;
            while h<=floor(end_time)
                ...
                if(outage_random_number < full_outage)
                    ...
                    gen_repair_time(i,1) = gen_outage_time(i,1) + GENVALUE.val(i,mttr);
                    %gen_repair_time(i,1) = gen_outage_time(i,1) + random('logn',log(GENVALUE.val(i,mttr)),0.5);
                    h=floor(end_time);
                end;
                h=h+1;
            end;

function [STARTINGUP_VAL,STARTUPMINGENHELP_VAL,SHUTTINGDOWN_VAL]=RTSCED_SUSD_Trajectories(UNITSTATUS,STATUS_VAL,gentype,UNITVALUE,ACTUAL_START_TIME,LASTRTSCEDBINDINGSCHEDULE,RTD_LOOKAHEAD_INTERVAL_VAL,INTERVAL_MINUTES_VAL,rtscuc_I_perhour,eps,su_time_index,sd_time_index,min_gen_index,initial_status_index,t,time,IDAC)

STARTUPMINGENHELP_VAL = 0;

%SU and SD trajectories
STARTINGUP_VAL = 0;
if (gentype ~= 7 && gentype ~= 10 && gentype ~= 14 && gentype ~= 16) && ((LASTRTSCEDBINDINGSCHEDULE+eps < UNITVALUE(1,min_gen_index)) ) 
    startup_period_check_end  = (max(1,ceil((RTD_LOOKAHEAD_INTERVAL_VAL(t,1)-UNITVALUE(1,su_time_index))*rtscuc_I_perhour+eps)+1));
    startup_period_check_time = ceil(RTD_LOOKAHEAD_INTERVAL_VAL(t,1)*rtscuc_I_perhour-eps) + 1;
    while(startup_period_check_time >= startup_period_check_end)
        ...
        if UNITSTATUS(startup_period_check_time,1)-Initial_RTD_last_startup_check == 1 && STATUS_VAL == 1 
...
if gentype ~= 7 && gentype ~= 10 && gentype ~= 14 && gentype ~= 16
    latest_shut_index = min(size(UNITSTATUS,1),ceil((RTD_LOOKAHEAD_INTERVAL_VAL(t,1) + UNITVALUE(1,sd_time_index))*rtscuc_I_perhour-eps)+1);
    ...
    while(shutdown_time_check_index < latest_shut_index)
        ...
        if Initial_RTD_last_shutdown_check - UNITSTATUS(shutdown_time_check_index,1) == 1

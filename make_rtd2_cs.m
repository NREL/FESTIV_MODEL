function [rtc_num, rtc_text] = make_rtd2_cs_(mo,month,date1, irtc, prtc)
% make SCUC and SCED files. Command line example:
%  for date=24:30 ; [~,~]=make_rtd2_cs_(7,'July',date, 5, 5) ; end
%%definitions:
 local = 'Input\'; % local directory
  remote = 'Y:\6A20\Public\RELM\FESTIV\solar_APS\clear_sky_APS\';
   %mo = 7;  month = 'July'; date1 = 24; % these are f-n inputs
    xl = '.xls'; csv = '.csv';
     vg_prefx = 'APS_wind_SC2_'; % needs mo/date to build the file name
      rtc_prefx = 'RTD2_APS_VG_forecast_'; rtc_suffx = '_at5min'; % needs mo/date...
       cs_prefx = 'CLEAR_Bus_'; cs_suffx = '_PV_5Min_SC1_2006_2020'; % needs BUS#... 
        %irtc=5; prtc=5; %hrtc=3;  hrtc and irtc are not needed (the left 2 columns of the file determine these parameters)
...
% read actual VG (one mins)
 vg_file = [local, vg_prefx, num2str(mo),'_',num2str(date1), xl];
  [vg_num, ~,~] = xlsread(vg_file); %('Input\APS_wind_SC2_7_24.xls');
    % select vg data
     nvg=size(vg_num,1); hours = vg_num(1:irtc:nvg,1); % at irtc intervals, from 1-min data
      vg_hourly = vg_num(1:irtc:nvg,:); vg_i0 = irtc-prtc; if vg_i0<1; vg_i0=1; end
       vg_hourly(2:size(vg_hourly,1),:)=vg_num(vg_i0:irtc:(nvg-irtc),:);
        vg_hourly_average=vg_hourly;
         for i=3:(size(vg_hourly,1)-1); i_from=(i-1)*irtc+0-floor(irtc/2); i_to=i_from+irtc-0;%two '0' in this line should be replaced with '1' -VD
          vg_hourly_average(i,:)=sum(vg_num(i_from:i_to,:),1)/(i_to-i_from+1);
         end % average

% read forecast file (for output format, reference)
 rtc_file = [local, rtc_prefx, month,num2str(date1),rtc_suffx,xl];
  [rtc_num, rtc_text,~] = xlsread(rtc_file);%('Input\RTD2_APS_VG_forecast_July24_at5min.xls');
         ...
         % find the relevant row in the clear sky matrix:
          while row<rows && ~condition; row=row+1; condition=(cs_num(row,2)==mo && cs_num(row,3)==date1); end; 
           today_cs_ = cs_num(row,4:size(cs_num,2)); % the whole row of data for the day
            n_cs=size(today_cs_,2); ncs_per_hr= n_cs/24; ncs_per_i=ncs_per_hr*irtc/60;
    ...
    time_from = rtc_num(i,1); time_to = rtc_num(i,2);
     i0=0;c=(1==0);while i0<size(hours,1)&&~c;i0=i0+1;c=(hours(i0)==time_from);end
      i1=0;c=(1==0);while i1<size(hours,1)&&~c;i1=i1+1;c=(hours(i1)==time_to);end
       perf = vg_hourly_average(i1, j-1); persis = vg_hourly(i0, j-1);
         ...
         rtc_num(i,j) = persis; % default forecast: persistent
          if strcmp('CSP',vg_type); rtc_num(i,j)=perf; end % perfect forecast for CSP
           if strcmp('PV',vg_type) % include clear sky index forecast for PV
            % it looks like the 5-min clear sky files need a 2 hr shift:
             twohrs=2*60/irtc;if i0>twohrs; i0=i0-twohrs;i1=i1-twohrs;end
...
% save modified forecast file:
 new_file = [local, rtc_prefx, month,num2str(date1),rtc_suffx,'_modc',xl];
